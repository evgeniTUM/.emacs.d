...
(setq-default mode-line-format 
              '("%e"
                (:eval (propertize (number-to-string (window-number)) 'face 'helm-source-header))

                "(" ;; '%02' to set to 2 chars at least; prevents flickering
                (:eval (propertize "%02l" 'face 'font-lock-preprocessor-face)) ","
                (:eval (propertize "%02c" 'face 'font-lock-preprocessor-face)) 
                ") "
                ...
                (:eval (when (file-remote-p (buffer-file-name))
                         (propertize "@"
                                     'face 'font-lock-type-face
                                     'help-echo "Buffer is read-only")))  
                "] "
                ...
                (:eval (propertize smartrep-mode-line-string 'face 'font-lock-warning-face))
...
(my-set-key "C-c w W" 'winner-redo)
