...
(setq ac-completing-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\t" 'auto-complete)
    (define-key map (kbd "<s-tab>") 'ac-expand) 
    map))
#+end_src
...
#+begin_src emacs-lisp
(defvar evgeni/keys-minor-mode-map (make-sparse-keymap) "evgeni/keys-minor-mode keymap.")
...
(add-to-list 'emulation-mode-map-alists
             (cons 'evgeni/keys-minor-mode
                   evgeni/keys-minor-mode-map))
#+end_src
...
(evgeni/set-key "M-j" 'ace-jump-mode)
...
(evgeni/set-key "C-c j j" 'helm-imenu-anywhere)
...
(evgeni/set-key "C-c h b" 'helm-bookmarks)
...
(evgeni/set-key "C-c h I c" 'clojure-cheatsheet)
 #+end_src

** Internet bindings
#+begin_src emacs-lisp
(evgeni/set-key "C-c n w" 'webjump)
(evgeni/set-key "C-c n s" 'helm-google-suggest)
(evgeni/set-key "C-c n g" 'helm-google)

(evgeni/set-key "C-c n q" (lambda () (interactive) (command-execute 'howdoi-query)))
(evgeni/set-key "C-c n Q" (lambda () (interactive) (command-execute 'howdoi-query-line-at-point)))
#+end_src
...
*** File management
#+begin_src emacs-lisp
(evgeni/set-key "C-c f f" 'ffap)
(evgeni/set-key "C-c f n" 'evgeni/narrow-to-region-indirect-buffer)
...
(evgeni/set-key "C-c b h" 'helm-bm)
...
(define-key smartscan-map (kbd "M-s-'") 'smartscan-symbol-replace)
...
(add-hook 'org-mode-hook 
          (lambda () "setup org bindings"   
            ...
            (local-set-key (kbd "C-c h h") 'helm-org-headlines)))
#+end_src
...
(add-hook 'eshell-mode-hook 
          (lambda () "setup eshell bindings"   
            ...
            (local-set-key (kbd "C-c h h") 'helm-eshell-history)))
#+end_src
