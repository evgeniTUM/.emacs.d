...
* Scientific writing
** Textlint - tips and tricks
   #+BEGIN_SRC emacs-lisp
     (defun my-textlint-run ()
       (interactive)
       (unless (fboundp 'textlint-run)
           (add-to-list 'load-path (expand-file-name "~/.emacs.d/lisp/textlint"))
           (require 'textlint))
       (command-execute 'textlint-run))
   #+END_SRC
...
(add-hook 'magit-mode-hook
          (lambda ()
            (define-key magit-mode-map (kbd "C-c 1") 'delete-other-windows)
            (define-key magit-mode-map (kbd "C-c 2") 'split-window-vertically)
            (define-key magit-mode-map (kbd "C-c 3") 'split-window-horizontally)
            (define-key magit-mode-map (kbd "C-c 4"))))
#+end_src
...
(my-set-key "C-0" 'ace-jump-mode) 
(my-set-key "M-0" 'iy-go-to-char)
